...
console.log(`🔧 Cache is ${DISABLE_CACHE ? 'disabled' : 'enabled'} (DISABLE_CACHE=${env.DISABLE_CACHE})`);
...
export class CloudflareDurableObjectRPCDrizzleCache extends Cache {
	...
	override async get(key: string): Promise<any[] | undefined> {
		...
		if (DISABLE_CACHE) {
			console.log(`🚫 Cache GET disabled - Key: ${prefixedKey}`);
			return undefined;
		}
	 ...
	 * ```
	 */
	override async put(key: string, response: any, tables: string[], isTag: boolean, config?: CacheConfig): Promise<void> {
		const prefixedKey = this.buildCacheKey(key);

		if (DISABLE_CACHE) {
			console.log(`🚫 Cache PUT disabled - Key: ${prefixedKey}, Tables: [${tables.join(', ')}]`);
			return;
		}
		...
		try {
			...
			await Promise.race([
				this.durableObject.putCacheEntry(prefixedKey, serializedResponse, tables, ttl),
				new Promise((_, reject) => setTimeout(() => reject(new Error('Cache PUT RPC timeout after 10 seconds')), 10000)),
			]);
	...
	override async onMutate(params: MutationParams): Promise<void> {
		...
		if (DISABLE_CACHE) {
			console.log(`🚫 Cache INVALIDATION disabled - Tables: [${normalizedTables.join(', ')}], Tags: [${normalizedTags.join(', ')}]`);
			return;
		}
	...
	async getCacheStats(): Promise<CacheStats> {
		try {
			...
			console.log(`📊 Cache stats retrieved - Total: ${stats.totalEntries}, Expired: ${stats.expiredEntries}`);
			return stats;
		} catch (error) {
	...
	private normalizeInvalidationParams(params: MutationParams): {
		...
		const normalizedTags = params.tags ? (Array.isArray(params.tags) ? params.tags : [params.tags]) : [];
