...
export class DiscordAPI {
    ...
    getCommandOptions(interaction: APIInteraction): APIApplicationCommandInteractionDataBasicOption[] | undefined {
        ...
        return interaction.data.options?.filter(
            (option): option is APIApplicationCommandInteractionDataBasicOption =>
                'value' in option
        );
    }
    ...
    async editOriginalResponse(
        ...
        const response = await fetch(
            `${RouteBases.api}${Routes.webhookMessage(this.applicationId, interactionToken, '@original')}`,
            {
                method: 'PATCH',
                headers: {
    ...
    async createFollowupMessage(
        ...
        const response = await fetch(
            `${RouteBases.api}${Routes.webhook(this.applicationId, interactionToken)}`,
            {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
    ...
    public getItemTypeEmoji(itemType: string): string {
        switch (itemType.toLowerCase()) {
            ...
            case 'music': return '🎵';
            case 'spray': return '🎨';
            case 'contrail': return '💫';
            case 'pet': return '🐕';
            case 'loadingscreen': return '🖼️';
            case 'banner': return '🚩';
            case 'toy': return '🎮';
            default: return '🎯';
        }
