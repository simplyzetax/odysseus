...
app.all('/friends/api/v1/:accountId/friends/:friendId/alias', ratelimitMiddleware(), acidMiddleware, async (c) => {
	...
	if (method === 'PUT') {
		...
		await db
			.update(FRIENDS)
			.set({ alias: body })
			.where(and(eq(FRIENDS.accountId, c.var.accountId), eq(FRIENDS.targetId, friendId)));
...
app.get('/friends/api/public/friends/:accountId', ratelimitMiddleware(), acidMiddleware, async (c) => {
	...
	const acceptedFriends = await db
		.select()
		.from(FRIENDS)
		.where(and(eq(FRIENDS.accountId, c.var.accountId), eq(FRIENDS.status, 'ACCEPTED')));
	...
	const incomingRequests = await db
		.select()
		.from(FRIENDS)
		.where(and(eq(FRIENDS.targetId, c.var.accountId), eq(FRIENDS.status, 'PENDING')));
	...
	const outgoingRequests = await db
		.select()
		.from(FRIENDS)
		.where(and(eq(FRIENDS.accountId, c.var.accountId), eq(FRIENDS.status, 'PENDING')));
...
app.post('/friends/api/*/friends*/:receiverId', ratelimitMiddleware(), acidMiddleware, async (c) => {
	...
	const [incomingRequest] = await db
		.select()
		.from(FRIENDS)
		.where(and(eq(FRIENDS.accountId, receiverId), eq(FRIENDS.targetId, senderId), eq(FRIENDS.status, 'PENDING')));
	...
	if (incomingRequest) {
		...
		await db
			.update(FRIENDS)
			.set({ status: 'ACCEPTED' })
			.where(and(eq(FRIENDS.accountId, receiverId), eq(FRIENDS.targetId, senderId)));
		...
		const [outgoingRequest] = await db
			.select()
			.from(FRIENDS)
			.where(and(eq(FRIENDS.accountId, senderId), eq(FRIENDS.targetId, receiverId)));
...
app.get('/friends/api/v1/:accountId/summary', ratelimitMiddleware(), acidMiddleware, async (c) => {
	...
	const blockedUsers = await db
		.select()
		.from(FRIENDS)
		.where(and(eq(FRIENDS.accountId, c.var.accountId), eq(FRIENDS.status, 'BLOCKED')));
